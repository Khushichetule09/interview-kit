...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "max-h-72 scroll-py-1 data-empty:p-0 overflow-y-auto overscroll-contain",
        className
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "bg-card text-foreground flex h-[calc(--spacing(5.25))] w-fit items-center justify-center gap-1 rounded-none px-1.5 text-xs font-medium whitespace-nowrap has-data-[slot=combobox-chip-remove]:pr-0 has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50",
        className
      )}
      {...props}
    >
      ...
      {showRemove && (
        <ComboboxPrimitive.ChipRemove
          render={<Button variant="destructive" size="icon-xs" className="h-full m-0! border-none!" />}
